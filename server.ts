...
async function startServer() {
  ...
  const getAi = () => {
    ...
    const activeKey = keys.find(k => k.value && !k.value.includes("TODO") && k.value.length > 10);
    
    if (activeKey && activeKey.value) {
      console.log(`Using API Key from ${activeKey.name} (Length: ${activeKey.value.length}, Starts with: ${activeKey.value.substring(0, 4)})`);
      return new GoogleGenAI({ apiKey: activeKey.value });
    }

    throw new Error("API_KEY_MISSING: Kunci API tidak ditemukan. Silakan tambahkan GEMINI_API_KEY di menu Settings > Secrets di AI Studio.");
  };

  // API Routes (Same logic as Vercel functions)
  app.post("/api/analyze-pdf", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ inlineData: { mimeType: "application/pdf", data: pdfBase64 } }, { text: prompt }],
        config: {
